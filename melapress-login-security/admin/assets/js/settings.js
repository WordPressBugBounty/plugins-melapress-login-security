...
function addQuestion( promptInputVal ) {
	...
	var $li = jQuery( '<li>' ).addClass( 'question-list-item custom' );
	$li.append( jQuery( '<span>' ).addClass( 'dashicons dashicons-sort' ) );
	$li.append( jQuery( '<label>' ).text( promptInputVal ) );
	$li.append( jQuery( '<input>' ).attr({ type: 'checkbox', id: $id, name: 'mls_options[enabled_questions][' + $id + ']', value: promptInputVal, checked: true }) );
	$li.append( jQuery( '<a>' ).attr( 'href', '#remove' ).text( 'Remove' ) );
	jQuery( '#questions-wrapper' ).append( $li );
}
...
jQuery( 'document' ).ready( function( $ ) {
	...
	/**
	 * Show what an exemption costs only while somebody is exempt.
	 *
	 * The list is a wall of consequences that means nothing on a site with an
	 * empty exclusion list, so it follows the list rather than sitting there
	 * permanently. Driven from the stored value rather than from the rendered
	 * items: the hidden field is what gets saved, and it is the only place that
	 * knows about users added and removed again before the form is submitted.
	 */
	function toggle_exemption_effects() {
		var $effects = jQuery( '#mls-exemption-effects' );

		if ( ! $effects.length ) {
			return;
		}

		var stored = jQuery( '#ppm-exempted-users' ).val();
		var users = [];
		...
		if ( stored ) {
			try {
				...
				// Unparseable means we cannot say the list is empty, and the
				// warning is the safer thing to show.
				users = [ 1 ];
			}
		}

		$effects.toggle( users.length > 0 );
	}
	...
	// The stored value is written by PHP on load, so the two are already in
	// step; this covers a value arriving by any other route.
	toggle_exemption_effects();
	...
	function hideShowToggleables() {
		if ( jQuery( '[data-toggle-target]' ).length > 0 ) {
			jQuery( '[data-toggle-target]' ).each( function () {
				var $target = jQuery( jQuery( this ).attr( 'data-toggle-target' ) );

				/*
				 * Disable the inputs, not just their appearance.
				 *
				 * This used to add the `disabled` class and nothing else. That
				 * class resolves to `opacity: .5; pointer-events: none`, which
				 * stops a mouse but not a keyboard: the fields could still be
				 * reached with Tab, edited, and — having no disabled attribute —
				 * submitted with the form. Setting the property as well makes
				 * the state real, and lines these sections up with the ones that
				 * were already disabling their inputs properly.
				 *
				 * The toggles themselves are left alone, or a control nested in
				 * the area it governs would switch itself off for good.
				 */
				$target.addClass( 'disabled' );
				$target.find( ':input' ).not( this ).prop( 'disabled', true );

				/*
				 * Only enable when this toggle is itself operable.
				 *
				 * Toggles are visited in document order, so where one governed
				 * area contains another toggle, the outer pass has already
				 * disabled the inner one by the time it is reached. Without this
				 * check an inner toggle left checked from an earlier save would
				 * hand its own fields back while its parent was switched off.
				 * Nothing nests that way today; the rule is here so that adding
				 * a sub-section does not quietly reopen the hole.
				 */
				if ( this.checked && ! this.disabled ) {
					$target.removeClass( 'disabled' );
					$target.find( ':input' ).not( this ).prop( 'disabled', false );
				}
			});
		}
	}
	...
	jQuery( document ).on( 'click', '#c4wp-prompt-wrapper a', function( event ) {
		...
		if ( jQuery( wrapper ).hasClass( 'type-notice' ) ) {
			...
			if ( jQuery( currentButton ).attr( 'href' ) == '#confirm-prompt' ) {
				...
				var allowedCallbacks = { addQuestion: addQuestion };
				var cbName = jQuery( currentButton ).attr( 'data-cb' );
				if ( allowedCallbacks.hasOwnProperty( cbName ) ) {
					allowedCallbacks[ cbName ].call( this, promptInputVal );
				}
				jQuery( wrapper ).fadeOut( 300 );
				setTimeout( function() {
					jQuery( wrapper ).remove();
	...
	/**
	 * Whether the role tab currently on screen is set to inherit.
	 *
	 * The control exists only on a role tab, and only there does its `table`
	 * carry a non-empty data-id — the same condition the handler below uses to
	 * tell a role tab from the site-wide one.
	 */
	function roleInheritsPolicies() {
		var $inherit = jQuery( '#ppm_master_switch' );

		if ( ! $inherit.length || ! $inherit.parents( 'table' ).data( 'id' ) ) {
			return false;
		}

		return $inherit.is( ':checked' );
	}
	...
	jQuery('#ppm_master_switch').change(function() {
		if ( jQuery( this ).parents( 'table' ).data( 'id' ) !='' ) {
			if( jQuery(this).is(':checked') ) {
				...
				/*
				 * That blanket re-enable does not know which groups and
				 * sections are switched off, so each is asked to state its own
				 * position again.
				 */
				jQuery( '.mls-policy-group-toggle' ).each( function () {
		...
		// Re-apply policy group toggle states after master switch changes.
		jQuery( '.mls-policy-group-toggle' ).each( function() {
			var $toggle = jQuery( this );
			var targetSelector = $toggle.data( 'target' );
			var $content = jQuery( targetSelector );
			if ( ! $toggle.is( ':checked' ) ) {
				$content.addClass( 'mls-policy-group-disabled' );
				$content.find( 'input, select, button, textarea' ).not( '.mls-policy-group-toggle' ).attr( 'disabled', 'disabled' );
			}
		});
	...
	// Policy group toggles - enable/disable individual policy sections.
	jQuery( '.mls-policy-group-toggle' ).each( function() {
		var $toggle = jQuery( this );
		var targetSelector = $toggle.data( 'target' );
		var $content = jQuery( targetSelector );
		...
		function toggleGroupContent() {
			/*
			 * While a role inherits, nothing below belongs to it.
			 *
			 * This handler runs after the inherit switch has disabled the form,
			 * and its enable branch was undoing that for every group that
			 * happened to be switched on in the inherited policy — leaving the
			 * role's fields editable on a tab that has no policy of its own.
			 */
			if ( $toggle.is( ':checked' ) && ! roleInheritsPolicies() ) {
				$content.removeClass( 'mls-policy-group-disabled' );
				$content.find( 'input, select, button, textarea' ).not( '.mls-policy-group-toggle' ).removeAttr( 'disabled' );

				/*
				 * Opening a group re-enables everything inside it, including
				 * fields belonging to a section that is still switched off, so
				 * the per-section states have to be re-applied afterwards.
				 */
				hideShowToggleables();

				if ( typeof disable_enabled_inactive_users_options === 'function' ) {
		...
		$toggle.on( 'change', toggleGroupContent );
		toggleGroupContent();
	});
...
function disable_enabled_inactive_users_options() {
	jQuery( '.mls-inactive-setting-reset-pw-row, #ppmwp-inactive-setting-row' ).addClass( 'disabled' );
	jQuery( '.mls-inactive-setting-reset-pw-row :input,  #ppmwp-inactive-setting-row :input' ).prop( 'disabled', true );
	...
	if ( jQuery( '#ppm-inactive-users-enabled' ).prop('checked') ) {
		jQuery( '.mls-inactive-setting-reset-pw-row, #ppmwp-inactive-setting-row' ).removeClass( 'disabled' );
		jQuery( '.mls-inactive-setting-reset-pw-row :input,  #ppmwp-inactive-setting-row :input' ).prop( 'disabled', false );
	}
}
...
function updateQuestionMaxCount() {
	...
	// When no questions remain enabled, disable the feature automatically.
	var toggle = jQuery( '#ppm-enable-security-question' );
	var counter = jQuery( '#prompt-counter' );
	if ( count === 0 ) {
		counter.val( 0 ).attr( 'min', 0 ).removeAttr( 'required' );
		if ( toggle.is( ':checked' ) ) {
			toggle.prop( 'checked', false ).trigger( 'change' );
		}
	} else {
		// The floor cannot exceed the number of questions on offer: with a
		// single question enabled, min="2" against a value of 1 makes the field
		// HTML-invalid and the browser silently refuses to submit the form.
		var lowerBound = Math.min( 2, count );

		counter.attr( 'min', lowerBound ).attr( 'required', 'required' );
		if ( parseInt( counter.val(), 10 ) < lowerBound ) {
			counter.val( lowerBound );
		}
		if ( parseInt( counter.val(), 10 ) > count ) {
			counter.val( count );
		}
	}
}
