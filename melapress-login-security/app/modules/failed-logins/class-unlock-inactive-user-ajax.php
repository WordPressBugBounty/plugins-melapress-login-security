...
if ( ! class_exists( '\MLS\Ajax\UnlockInactiveUser' ) ) {
	...
	class UnlockInactiveUser implements AjaxInterface {
		...
		public function action() {
			...
			if ( ! $userdata ) {
				wp_send_json_error( array( 'message' => __( 'User not found.', 'melapress-login-security' ) ) );
			}

			$role_options = OptionsHelper::get_preferred_role_options( $userdata->roles );

			// Determine the original lock reason for notification purposes.
			$currently_unlocking_user_logins = ( isset( $_POST['unblocking_user'] ) && 'true' === $_POST['unblocking_user'] ) ? true : false;

			// R1 — Single unlock: fully clear ALL lock types at once.
			OptionsHelper::fully_unlock_user( $userdata->ID, $currently_unlocking_user_logins ? 'blocked' : 'inactive' );

			// Determine password reset and send notification based on original lock reason.
			...
			$reset_time = current_time( 'timestamp' ); // phpcs:ignore WordPress.DateTime.CurrentTimeTimestamp.Requested
