...
if ( ! class_exists( '\MLS\Migrate_UserMeta_BG_Process' ) ) {
	...
	class Migrate_UserMeta_BG_Process extends \WP_Background_Process {
		...
		protected function task( $item ) {
			...
			global $wpdb;
			$users_data = $wpdb->get_results( $wpdb->prepare( "SELECT meta_key FROM $wpdb->usermeta WHERE meta_key LIKE %s AND user_id = %d", array( $wpdb->esc_like( 'ppmwp_' ) . '%', $user->ID ) ), ARRAY_A ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
