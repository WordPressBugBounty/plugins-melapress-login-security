...
if ( ! class_exists( '\MLS\Helpers\UserImporter' ) ) {
	...
	class UserImporter {
		...
		public function init() {
			...
			\add_action( 'wp_ajax_mls_export_users', array( $this, 'export_users' ) );
			\add_action( 'wp_ajax_mls_process_user_import', array( $this, 'process_import' ) );
		...
		public static function render_settings() {
			...
			/*
			 * get_editable_roles(), not every role on the site: it is what core
			 * uses for the same dropdown on user-new.php, and it excludes roles
			 * the current user is not allowed to hand out. On multisite that
			 * matters, because a site administrator could otherwise create an
			 * account with any role the network defines.
			 */
			// Lives in wp-admin/includes/user.php, which is not loaded on
			// every request that can reach here.
			...
			foreach ( get_editable_roles() as $role_slug => $role_details ) {
				$roles[ $role_slug ] = translate_user_role( $role_details['name'] );
			}
			?>
										...
										<div id="file-import-settings">
											...
											<fieldset>
												...
												foreach ( $roles as $key => $value ) {
													/*
													 * The slug is the value; the
													 * display name is only the
													 * label. This used to submit
													 * the name, which the handler
													 * lowercased — so it happened
													 * to work for single-word
													 * built-in roles and silently
													 * assigned nothing at all for
													 * anything else, "Shop Manager"
													 * becoming "shop manager".
													 */
													if ( 'subscriber' === $key ) {
		...
		public function export_users() {
			...
			// Check nonce. `list_users` is the capability that actually governs
			// reading the user list this exports.
			if ( ! current_user_can( 'manage_options' ) || ! current_user_can( 'list_users' ) || empty( $nonce ) || ! wp_verify_nonce( $nonce, 'mls-export-settings' ) ) {
			...
			foreach ( $blogusers as $user ) {
				$results[] = self::to_csv_row( array( $user->user_login, $user->user_email ) );
			}
		...
		/**
		 * Render one CSV row from a list of fields.
		 *
		 * Fields used to be joined with a bare comma. Two things went wrong with
		 * that. A value containing a comma or a quote — both legal in a display
		 * name, and `user_email` is not as constrained as it looks — silently
		 * shifted every following column. And a value starting with `=`, `+`,
		 * `-`, `@`, tab or carriage return is treated as a formula by Excel,
		 * LibreOffice and Google Sheets, so an account named `=cmd|…` runs when
		 * an administrator opens the export they just downloaded. Neither is
		 * exotic: usernames are chosen by whoever registers.
		 *
		 * Quoting handles the first. Prefixing a leading formula character with
		 * an apostrophe handles the second, and is what the spreadsheet
		 * applications themselves recommend.
		 *
		 * @param string[] $fields Ordered field values.
		 *
		 * @return string
		 *
		 * @since 2.4.0
		 */
		private static function to_csv_row( array $fields ) {
			$escaped = array();

			foreach ( $fields as $field ) {
				$field = (string) $field;

				if ( '' !== $field && false !== strpos( "=+-@\t\r", $field[0] ) ) {
					$field = "'" . $field;
				}
		...
		public function process_import() {
			...
			// Check nonce.
			//
			// `create_users` and `promote_users` matter as well as
			// `manage_options`: on multisite core withholds `create_users` from
			// site administrators unless the network allows it, so gating on
			// `manage_options` alone let them create accounts the network
			// administrator had deliberately forbidden. This mirrors the check
			// Temporary_Logins::create_new_user() already performs.
			if ( ! current_user_can( 'manage_options' ) || ! current_user_can( 'create_users' ) || ! current_user_can( 'promote_users' ) || empty( $nonce ) || ! wp_verify_nonce( $nonce, 'mls-export-settings' ) || empty( $_POST['username'] ) ) {
				wp_send_json_error( esc_html__( 'Nonce Verification Failed.', 'melapress-login-security' ) );
			}

			$new_user_role = isset( $_POST['role'] ) && ! empty( $_POST['role'] ) ? \sanitize_key( \wp_unslash( $_POST['role'] ) ) : 'subscriber';

			/*
			 * The role arrives in the request, so it is checked rather than
			 * trusted. get_editable_roles() is the same gate core applies on
			 * user-new.php; without it this endpoint would assign any role the
			 * site defines, including one the caller is not permitted to grant.
			 */
			// Lives in wp-admin/includes/user.php, which is not loaded on
			// every request that can reach here.
			if ( ! function_exists( 'get_editable_roles' ) ) {
			...
			if ( ! array_key_exists( $new_user_role, get_editable_roles() ) ) {
				wp_send_json_error( esc_html__( 'You cannot assign the requested role.', 'melapress-login-security' ) );
				exit;
			}
			...
			if ( ! username_exists( $username ) ) {
				...
				if ( is_wp_error( $user_id ) ) {
					...
					if ( $email_address ) {
						...
						/*
						 * A password-set link, never the password itself.
						 *
						 * The non-forced branch used to pass the generated
						 * password to the template, which put the account's real
						 * credential into an email: plaintext in transit unless
						 * every hop negotiates TLS, then resident in the
						 * recipient's mailbox, the sending log and any archive
						 * for as long as those exist, and unrevocable once sent.
						 * Core stopped doing this in WordPress 4.3. Both branches
						 * now send the same kind of link; they differ only in
						 * which template they use and whether a reset is forced
						 * on first login.
						 */
						$reset_url = self::build_password_set_url( $user );
						...
						$email_content = \MLS\EmailAndMessageStrings::replace_email_strings( $message, $user_id, array( 'reset_url' => $reset_url ) );
						$email_content = self::ensure_route_in( $email_content, $reset_url );

						Emailer::send_email( $email_address, wp_specialchars_decode( $title ), $email_content, $headers );
					}
		...
		/**
		 * Build the URL at which a newly imported user sets their own password.
		 *
		 * @param \WP_User $user User the link is for.
		 *
		 * @return string URL, or an empty string if a key could not be issued.
		 *
		 * @since 2.4.0
		 */
		private static function build_password_set_url( $user ) {
			$key = get_password_reset_key( $user );

			/*
			 * This returns a WP_Error when the `allow_password_reset` filter
			 * declines — which this plugin's own "disable password resets"
			 * setting can cause. The old code interpolated the result into the
			 * URL regardless, producing a link that could not work. An empty
			 * string is returned instead, and ensure_route_in() below tells the
			 * recipient who to ask.
			 */
			...
			return esc_url_raw(
				network_site_url(
					$login_page . '?action=rp&key=' . rawurlencode( $key ) . '&login=' . rawurlencode( $user->user_login ),
					'login'
				)
			);
		}
		...
		/**
		 * Make sure the message leaves the recipient a way into the account.
		 *
		 * The body comes from a template an administrator may have edited. One
		 * customised before this change is likely to say "Password:" followed by
		 * a placeholder that now resolves to nothing, which would strand every
		 * imported user with an email that tells them neither their password nor
		 * where to set one. Rather than rewrite their template, the missing line
		 * is appended.
		 *
		 * @param string $body      Rendered message.
		 * @param string $reset_url Password-set URL, or an empty string.
		 *
		 * @return string
		 *
		 * @since 2.4.0
		 */
		private static function ensure_route_in( $body, $reset_url ) {
			$body = (string) $body;

			if ( '' === (string) $reset_url ) {
				return rtrim( $body ) . "\n\n" . esc_html__(
					'Please contact the website administrator to have your password set.',
					'melapress-login-security'
				) . "\n";
			}

			if ( false !== strpos( $body, $reset_url ) ) {
				return $body;
			}

			return rtrim( $body ) . "\n\n" . sprintf(
				/* translators: %s: URL at which the user sets their password. */
				esc_html__( 'To set your password and log in, visit the following URL: %s', 'melapress-login-security' ),
				$reset_url
			) . "\n";
		}
			...
			<table class="form-table has-sticky-bar">
				<tbody>
					<tr valign="top">
						...
						<p class="description"><?php esc_html_e( 'This email is sent to notify users when a user account has been created on the website with their email address.', 'melapress-login-security' ); ?></p>
					</tr>
			...
			<table class="form-table has-sticky-bar">
				<tbody>
					<tr valign="top">
						...
						<p class="description"><?php esc_html_e( 'This email is sent to notify users when a user account has been created on the website with their email address and must reset the password before the log in to the website.', 'melapress-login-security' ); ?></p>
					</tr>
