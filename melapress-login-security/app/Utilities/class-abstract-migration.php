 ...
 */
if ( ! class_exists( '\MLS\Abstract_Migration' ) ) {

	/**
	 * Utility class to ease the migration process.
	 *
	 * Every migration must go in its own method.
	 * The naming convention is migrate_up_to_XXX where XXX is the version number
	 * with dots removed.
	 * Example: migration for version up to 2.5.0 must be in migrate_up_to_250 method
	 *
	 * The numbers in the names of the methods must have exact numbers count as in the selected
	 * version in use, even if there are silent numbers for some of the major versions as 1, 2, 3 etc.
	 * Example:
	 *  - if X.X.X is selected for version number, then for version 1.1 method must have "...migrate_up_to_110..." in its name
	 *  - if X.X is selected for version number, then for version 1, method must have "...migrate_up_to_10..." in its name
	 *
	 * Note: you can add prefix to the migration method, if that is necessary, but "migrate_up_to_" is a must -
	 * the name must contain that. @see get_all_migration_methods_as_numbers of that class.
	...
	class Abstract_Migration {
		...
		private static function get_stored_version() {
			...
			if ( '' === trim( (string) static::$stored_version ) ) {
				static::$stored_version = (string) self::get_option( static::$version_option_name, '0.0.0' );
			}
		...
		private static function store_updated_version() {
			self::update_option( static::$version_option_name, \constant( static::$const_name_of_plugin_version ) );

			if ( '0.0.0' !== (string) static::$stored_version ) {
				self::update_option( self::UPGRADE_NOTICE, 1 );
			}
		}
		...
		/**
		 * Gets an option value with multisite support.
		 *
		...
		/**
		 * Updates an option value with multisite support.
		 *
		 * @param string $option_name The name of the option.
		...
		/**
		 * Deletes an option with multisite support.
		 *
		 * @param string $option_name The name of the option.
		 *
		 * @return bool
