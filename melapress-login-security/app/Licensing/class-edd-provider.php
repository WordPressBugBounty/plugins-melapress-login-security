<?php
/**
 * Easy Digital Downloads (EDD) Licensing Provider for Melapress Login Security plugin.
 *
...
if ( ! class_exists( '\MLS\Licensing\EDD_Provider' ) ) {
	...
	class EDD_Provider implements Licensing_Provider {
		 ...
		 * @var string
		 */
		const LICENSE_KEY_OPTION = 'mls_edd_license_key';
		 ...
		 * @var string
		 */
		const LICENSE_STATUS_OPTION = 'mls_edd_license_status';
		 ...
		 * @var string
		 */
		const LICENSE_DATA_OPTION = 'mls_edd_license_data';
		 ...
		 * @var string
		 */
		const LICENSE_CHECK_TRANSIENT = 'mls_edd_license_check';
		...
		public static function init() {
			...
			add_action( 'wp_ajax_mls_edd_activate_license', array( __CLASS__, 'ajax_activate_license' ) );
			add_action( 'wp_ajax_mls_edd_deactivate_license', array( __CLASS__, 'ajax_deactivate_license' ) );
		}
		...
		public static function activate_license( string $license_key ) {
			...
			$error_message = isset( $license_data['error'] ) ? $license_data['error'] : __( 'License activation failed.', 'melapress-login-security' );
		...
		public static function is_available(): bool {
			...
			return apply_filters( 'mls_edd_provider_available', true );
		}
		...
		public static function license_notices() {
			...
			if ( 'expired' === $status ) {
				...
				printf(
					...
					esc_html__( 'Your Melapress Login Security license has expired. Please %s to renew your license and continue receiving updates and support.', 'melapress-login-security' ),
					'<a href="' . esc_url( self::get_account_url() ) . '" target="_blank">' . esc_html__( 'renew your license', 'melapress-login-security' ) . '</a>'
				);
				echo '</p></div>';
		...
		public static function ajax_activate_license() {
			check_ajax_referer( 'mls_edd_license', 'nonce' );
