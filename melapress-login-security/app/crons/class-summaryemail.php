...
if ( ! class_exists( '\MLS\Crons\SummaryEmail' ) ) {
	...
	class SummaryEmail implements CronInterface {
			...
			add_filter(
				...
				function ( $schedules ) {
					$schedules['weekly'] = array(
						'interval' => 604800,
						'display'  => __( 'Once Weekly', 'melapress-login-security' ),
					);
					return $schedules;
				}
			);
		...
		public function send_summary_email() {
			...
			if ( wp_doing_ajax() ) {
				check_ajax_referer( 'mls_send_summary_email' );
			...
			$blocked_users  = array();
				...
				$blocked_users      = \MLS\Failed_Logins::get_all_currently_login_locked_users();
			...
			Emailer::send_email( $email, wp_specialchars_decode( $title ), $message, $headers );
		}
